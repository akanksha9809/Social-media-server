...
const signupController = async(req,res) => {
    try {
        ...
        if(oldUser) {
            return res.status(409).send("User is alredy registered, login with this id");    //conflict status code
        }
...
const loginController = async(req,res) => {
    try {
        ...
        if(!user) {
            return res.status(404).send("User is not registered, please signup!");    
        }
...
const refreshAccessTokenController = async(req, res) => {
    ...
    if(!refreshToken) {
        return res.status(401).send("Refersh token is required");
    }
